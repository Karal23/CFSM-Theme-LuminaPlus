...
- **后端探测窗口 2026-08-23 换了实现，老的那套坑基本作废**（站长口径：改成从 D1 取窗口的
  ...
  **2026-08-24 站长又把窗口跨度从 1 小时拉到 2 小时**（还是 20 个点，步长≈6 分钟；D1 消耗略大、
  ...
  - 旧实现（2026-08-18 读 `huilang-me/CF-Server-Monitor` 源码定位）是 `MetricsBroadcaster.js`
    ...
    留着只为解释旧版本的历史截图。
  - **2026-09-24 已核对当前部署的后端源码和 D1 实测**：`getDashboardLatencyHistory` 按时间段
    从历史表各取一个真实记录，缺测返回空槽；相邻记录数值相同不代表复制。海创到阿里云的
    20 个窗口点均为真实的 2ms / 0%，被旧 `dropBackfilledRuns` 整段误删，详情回灌后才补齐。
    已移除该猜测逻辑。真实缺测仍由空槽表达，不能为了补满柱子而新增自动历史查询或虚构数据。
